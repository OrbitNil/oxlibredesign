...
const useStyles = createStyles((theme, params: { position?: MenuPosition; itemCount: number; selected: number }) => ({
  ...
  container: {
    ...
    // Same as title/list — if lighter bg bleeds through subpixel gaps in CEF it reads as a thin grey line
  ...
  buttonsWrapper: {
    ...
    maxHeight: 340,
    overflow: 'hidden',
    borderBottomLeftRadius: params.itemCount <= 6 || params.selected === params.itemCount - 1 ? theme.radius.md : 0,
    borderBottomRightRadius: params.itemCount <= 6 || params.selected === params.itemCount - 1 ? theme.radius.md : 0,
    // Same as Header titleBar — avoids visible seam / “grey bar” at the join
    backgroundColor: 'rgba(8, 8, 8, 0.78)',
    borderTop: 'none',
      ...
      {visible && (
        ...
        >
          <Box className={classes.container}>
            <Header title={menu.title} bannerImage={menu.bannerImage} titlePrefix={menu.titlePrefix} />
            <Box className={classes.buttonsWrapper} onKeyDown={(e: React.KeyboardEvent<HTMLDivElement>) => moveMenu(e)}>
              <FocusTrap active={visible}>
                <Stack spacing={0} sx={{ overflowY: 'scroll' }}>
